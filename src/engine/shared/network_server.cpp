...
int CNetServer::Update()
{
	...
	for(int i = 0; i < MaxClients(); i++)
	{
		...
		if(m_aSlots[i].m_Connection.State() == NET_CONNSTATE_ERROR)
		{
			...
			{
				if(NetBan()->BanAddr(ClientAddr(i), 180, "Stressing network") == -1)
	...
	while(1)
	{
		...
		if(CNetBase::UnpackPacket(m_RecvUnpacker.m_aBuffer, Bytes, &m_RecvUnpacker.m_Data) == 0)
		{
			...
			else
			{
				...
				if(m_RecvUnpacker.m_Data.m_Flags&NET_PACKETFLAG_CONTROL && m_RecvUnpacker.m_Data.m_aChunkData[0] == NET_CTRLMSG_CONNECT && m_RecvUnpacker.m_Data.m_DataSize != 0)
				{
					bool Found = false;
